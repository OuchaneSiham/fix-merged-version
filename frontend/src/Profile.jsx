...
function Profile() {
    ...
    const handleSave = async () => {
        ...
        if (updatedData.password && updatedData.password.trim() !== "") {
            if (!updatedData.currentPassword || updatedData.currentPassword.trim() === "") {
    ...
    const startChatWith = async (friendId) => {
        ...
        try {
            const res = await fetch("https://localhost:8443/api/v1/chat/conversation", {
                method: "POST",
                headers: {
                    "Content-Type": "application/json",
        ...
        <div style={{ padding: "20px" }}>
            ...
            {userData.role === "admin" && (
                <div style={{ background: "#f0f0f0", padding: "10px", borderRadius: "8px", marginTop: "10px" }}>
                    ...
                    <Link to="/admin"><button style={{ marginLeft: "10px" }}>Go to Admin Dashboard</button></Link>
                </div>
            )}
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "20px" }}>
                <div>
                    ...
                    {friends.length === 0 && <p style={{color: "#999"}}>No friends yet. Search for users to add!</p>}
                    {friends.map(fr => (
                ...
                <div>
                    ...
                    {pendingReqs.length === 0 && <p style={{color: "#999"}}>No pending requests</p>}
                    {pendingReqs.map(req => (
...
<button onClick={handleLogout} style={{ marginTop: "30px", color: "red" }}>Logout</button>
            <button onClick={handleLogout} style={{ marginTop: "30px", color: "red" }}>Logout</button>
        </div>
    );
}
